...
class FolderScannerApp:
            ...
            try:
                if action['type'] == 'file_rename':
                    ...
                    for file_info in deleted_files:
                        try:
                            trash_path = self.get_trash_path(file_info['original_path'])
                            if trash_path and os.path.exists(trash_path):
                                # Восстанавливаем из корзины
                                ctypes.windll.shell32.SHFileOperationW.restore_from_trash = True
                                restored_count += 1
                        except:
                            continue
                    ...
                    if restored_count > 0:
                        self.scan_selected()
                        self.status_var.set(f"Восстановлено файлов: {restored_count}")
                    ...
                    for file_info in files_to_delete:
                        try:
                            if os.path.exists(file_info['original_path']):
                                send2trash(file_info['original_path'])
                                deleted_count += 1
                        except:
                            continue
    ...
    def get_trash_path(self, file_path):
        """Получить путь файла в корзине"""
        # Это упрощенная версия, в реальности нужно использовать SHQueryRecycleBin
        return file_path
    ...
    def delete_selected(self):
        ...
        if messagebox.askyesno("Подтверждение", f"Удалить {len(files_to_delete)} выбранных файлов в корзину?"):
            deleted_files = []
            for file_info in files_to_delete:
                try:
                    send2trash(file_info['original_path'])
            ...
            if deleted_files:
                ...
                self.scan_selected()
                self.status_var.set(f"Удалено файлов в корзину: {len(deleted_files)}")
