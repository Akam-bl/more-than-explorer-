...
class FolderScannerApp:
                ...
                for file_info in files:
                    ...
                    if temp_path and os.path.exists(temp_path):
                        try:
                            ...
                            self.status_var.set(f"Восстановлен файл: {os.path.basename(file_path)}")
                        except Exception as e:
                ...
                for file_info in files:
                    ...
                    if os.path.exists(file_path):
                        try:
                            ...
                            self.status_var.set(f"Удален файл: {os.path.basename(file_path)}")
                        except Exception as e:
    ...
    def delete_selected(self):
        ...
        for item in selected_items:
            ...
            if item_values and len(item_values) >= 3:
                ...
                if item_type == 'file':
                    ...
                    if os.path.exists(file_path):
                        # Создаем временную копию для возможности восстановления
                        ...
                        try:
                            ...
                            messagebox.showerror("Ошибка", f"Не удалось создать резервную копию файла: {str(e)}")
                            return
        ...
        if messagebox.askyesno("Подтверждение", f"Удалить {len(files_to_delete)} выбранных файлов?"):
            try:
                # Используем Windows API для удаления в корзину
                from win32com.shell import shell, shellcon
                ...
                self.refresh_tree()
                self.status_var.set(f"Удалено файлов: {len(files_to_delete)} (можно отменить Ctrl+Z)")
                
            except ImportError:
                # Резервный вариант если нет win32com
                for file_info in files_to_delete:
                    try:
    ...
    def create_widgets(self):
        ...
        # Убрана кнопка "Скрыть дочерние BB" с панели инструментов
        ...
        if item:
            ...
            if item_values and len(item_values) >= 3:
                ...
                if item_type == 'file':
                    ...
                    self.tree_menu.add_command(label="Открыть с помощью...", command=self.open_file_with_windows_dialog)
    ...
    def rename_virtual_folder(self, folder_name):
        ...
        if new_name and new_name != folder_name:
            ...
            # Сохраняем старые файлы для истории
            old_files = self.virtual_folders[folder_name].copy()
        ...
        if item:
            ...
            if item_values and len(item_values) >= 3:
                ...
                if item_type == 'virtual_folder':
                    # Открываем виртуальную папку (раскрываем содержимое)
                    current_state = self.tree.item(item, 'open')
                    self.tree.item(item, open=not current_state)
                elif item_type == 'file':
                    file_path = item_values[0]
                    self.open_file_with_windows_dialog(file_path)
                elif item_type == 'folder':
                    # Переключаем состояние развертывания обычной папки
                    current_state = self.tree.item(item, 'open')
                    self.tree.item(item, open=not current_state)
    
    def open_file_with_windows_dialog(self, file_path=None):
        """Открыть файл через стандартный диалог Windows 'Открыть с помощью'"""
        ...
        try:
            # Используем Windows Shell API для открытия диалога "Открыть с помощью"
            import win32api
            import win32con
            import win32ui
            import pythoncom
            
            # Подготавливаем параметры для ShellExecuteEx
            sei = win32ui.ShellExecuteInfo()
            sei.fMask = win32con.SEE_MASK_NOCLOSEPROCESS
            sei.lpVerb = 'openas'  # 'openas' открывает диалог "Открыть с помощью"
            sei.lpFile = actual_file_path
            sei.nShow = win32con.SW_SHOW
            
            # Выполняем
            sei.ShellExecute()
            
            self.status_var.set(f"Открытие файла через диалог 'Открыть с помощью': {os.path.basename(file_path)}")
            
        except ImportError:
            # Резервный способ для систем без win32api
            try:
                # Пытаемся использовать PowerShell для открытия диалога
                subprocess.run([
                    'powershell',
                    '-Command',
                    f'Start-Process -FilePath "{actual_file_path}" -Verb OpenAs'
                ...
                try:
                    ...
                    messagebox.showerror("Ошибка", f"Не удалось открыть файл: {str(e2)}")
        except Exception as e:
            messagebox.showerror("Ошибка", f"Не удалось открыть диалог 'Открыть с помощью': {str(e)}")
    ...
    def open_file_location(self):
        ...
        if len(item_values) >= 1:
            ...
            if os.path.exists(file_path):
                try:
                    # Используем Windows API для открытия проводника с выделенным файлом
                    import subprocess
                    ...
                    subprocess.run(['explorer', '/select,', file_path], shell=True, check=False)
                    
                    self.status_var.set(f"Открыта папка с выделенным файлом: {os.path.basename(file_path)}")
                    
                except Exception as e:
                    # Резервный способ - просто открываем папку
                    try:
    ...
    def rename_file(self):
        """Переименовать файл с поддержкой отмены"""
        selected_items = self.tree.selection()
        if not selected_items:
            return
    ...
    def create_menu(self):
        ...
        # Перенесена опция "Скрыть дочерние файлы Blockbench"
        ...
        view_menu.add_checkbutton(label="Группировать дубликаты", 
                                 variable=self.hide_duplicates_var,
                                 command=self.on_hide_duplicates_changed)
        # Убрана опция "Скрыть дочерние файлы Blockbench" из меню "Вид"
...
🎨 Автоматически открывает 3D-модели в Blockbench с текстурами
