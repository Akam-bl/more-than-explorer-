...
class CustomTreeWidget(QTreeWidget):
    ...
    def mousePressEvent(self, event):
        if event.button() == Qt.LeftButton:
            ...
            if item:
                ...
                if event.modifiers() & Qt.ControlModifier:
                    if item in self.last_selection_state:
                        ...
                        self.last_selection_state.remove(item)
                        self.setCurrentItem(None)
                    else:
                        # Если не выделен - добавляем
                        self.last_selection_state.add(item)
                    ...
                    if self.last_clicked_item:
                        ...
                        if start_index >= 0 and end_index >= 0:
                            ...
                            for i in range(min_idx, max_idx + 1):
                                selected_items.add(all_items[i])
                            
                            # Объединяем с предыдущим выделением
                            selected_items.update(self.last_selection_state)
            ...
            # Начало выделения перетаскиванием (только если не Ctrl/Shift и не на элементе)
            if not item and not (event.modifiers() & (Qt.ControlModifier | Qt.ShiftModifier)):
                ...
                # Запоминаем начальное состояние выделения
                self.last_selection_state = set()
...
class FolderScannerApp(QMainWindow):
    def __init__(self):
        ...
        # Новые переменные для расширенного поиска
        self.search_modes = ["name", "content", "date", "size"]
        self.current_search_mode = self.settings.get("search_mode", "name")
        self.multicriteria_search_enabled = False
        self.search_criteria = {"name": "", "content": "", "date": "", "size": ""}
    ...
    def init_ui(self):
        ...
        # Панель инструментов (НЕ скрываемая)
        self.create_toolbar()
        ...
        # Правый угол: прогресс бар (аналогично версии 5.10)
    ...
    def create_toolbar(self):
        ...
        # Кнопки отмены/повтора (используем символы из версии 5.10)
        ...
        # Поиск с расширенными режимами
        search_label = QLabel("Поиск:")
        search_label.setToolTip("Поиск файлов по различным критериям")
        toolbar.addWidget(search_label)
        
        # Кнопка переключения режима поиска с иконками
        ...
        self.multicriteria_btn.setChecked(self.settings.get("multicriteria_search", False))
    ...
    def create_menu(self):
        ...
        # Скрыть дочерние файлы Blockbench перемещено в спец настройки
    ...
    def apply_theme(self):
        """Применение темы (светлая/тёмная) с мягкой палитрой"""
        app = QApplication.instance()
        
        if self.dark_mode:
            # Тёмная тема с мягкой палитрой
            ...
            # Стили для виджетов с мягкими цветами
            dark_stylesheet = """
            QMainWindow {
            ...
            # Светлая тема с мягкой палитрой
            light_palette = QPalette()
            
            # Мягкие цвета для светлой темы
            ...
            QTreeWidget::item:selected {
                background-color: rgba(70, 130, 220, 100);
                color: white;
                border: 1px solid #4682dc;
            }
            QTreeWidget::item:hover {
                background-color: rgba(70, 130, 220, 30);
            }
            QGroupBox {
                color: #1e1e20;
                border: 1px solid #d0d0d2;
            ...
            QMenuBar::item:selected {
                background-color: rgba(70, 130, 220, 50);
            }
            QMenu {
                background-color: white;
                color: #1e1e20;
    ...
    def toggle_multicriteria_search(self, enabled):
        ...
        if enabled:
            ...
            self.toggle_search_mode()  # Сбросить на текущий режим
        self.save_settings()
        self.refresh_files_tree()
        ...
        if criteria["size"]:
            ...
            if match:
                ...
                for unit_name, multiplier in units.items():
                    if search_size.upper().endswith(unit_name):
    ...
    def refresh_files_tree(self):
        ...
        for file_info in self.all_files:
            ...
            if self.hide_bb_children_cb_menu.isChecked():
    ...
    def show_tree_context_menu(self, position):
        ...
        if item_type == 'file':
            ...
            # Удалить папку (с предупреждением)
    ...
    def create_virtual_folder_from_selection(self):
        ...
        if ok and folder_name:
            ...
            # Проверяем дубликаты содержимого (упрощенно)
            content_warning = False
            if len(files_to_add) > 1:
                # Можно добавить более сложную проверку содержимого
                pass
            ...
            if content_warning:
                QMessageBox.warning(self, "Предупреждение", 
                                  "В папке есть файлы с похожим содержимым. Проверьте необходимость их дублирования.")
    ...
    def open_file_with_dialog_win11(self, file_path):
        ...
        try:
            if sys.platform == "win32":
                ...
                # Этот метод работает на всех версиях Windows, включая 11
                subprocess.run([
                    "rundll32.exe", 
