...
class FolderScannerApp:
            ...
            if action['type'] == 'file_rename':
                ...
                if os.path.exists(new_path) and not os.path.exists(old_path):
                    try:
                        ...
                        self.refresh_tree()  # Обновление дерева после отмены
            ...
            if action['type'] == 'file_rename':
                ...
                if os.path.exists(old_path) and not os.path.exists(new_path):
                    try:
                        ...
                        self.refresh_tree()  # Обновление дерева после повтора
        ...
        if item:
            ...
            if item_values and len(item_values) >= 3:
                ...
                if item_type == 'virtual_folder':
                    # Только toggle состояния папки (раскрыть/свернуть)
    ...
    def open_file(self, file_path):
        ...
        self.open_file_with_dialog(actual_file_path)
    ...
    def open_file_with_dialog(self, file_path):
        ...
        try:
            # Для Windows используем ShellExecute с командой "openas"
            ...
            if result <= 32:
                # Если ShellExecute не сработал, пробуем стандартный способ
                try:
    ...
    def create_menu(self):
        ...
        # Кнопка "Группировать дубликаты" удалена отсюда (перемещена в настройки)
    ...
    def show_hotkeys(self):
        ...
        hotkeys_window.title("Сочетания клавиш")
        hotkeys_window.geometry("400x300")
        hotkeys_window.transient(self.root)
        hotkeys_window.grab_set()
        
        # Создаем таблицу
        tree = ttk.Treeview(hotkeys_window, columns=("action", "shortcut"), show="headings")
        tree.heading("action", text="Действие")
        tree.heading("shortcut", text="Сочетание")
        ...
        hotkeys = [
            ...
            ("Отменить", "Ctrl+Z"),
            ("Повторить", "Ctrl+Y"),
            ("Создать виртуальную папку", "Ctrl+J")
        ]
