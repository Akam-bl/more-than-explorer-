...
class FolderScannerApp:
    ...
    def open_file(self, file_path):
        ...
        try:
            ...
            if actual_file_path.lower().endswith(('.json', '.bbmodel')):
                self.open_model_with_textures(actual_file_path)
            else:
                # Для остальных файлов стандартное открытие
                if os.name == 'nt':  # Windows
                    os.startfile(actual_file_path)
                elif os.name == 'posix':  # macOS, Linux
                    subprocess.run(['open' if sys.platform == 'darwin' else 'xdg-open', actual_file_path])
                self.status_var.set(f"Открытие файла: {os.path.basename(file_path)}")
        except Exception as e:
            messagebox.showerror("Ошибка", f"Не удалось открыть файл: {str(e)}")
        finally:
            # Очищаем временный файл после использования (если он был создан)
            if temp_file_path and os.path.exists(temp_file_path):
                ...
                # Отложенная очистка через 30 секунд
    ...
    def open_file_location(self):
        ...
        file_path = item_values[0]
        archive_path = item_values[1] if len(item_values) > 1 else ''
        
        # Если файл находится в архиве
        if archive_path and os.path.exists(archive_path):
            try:
                # Открываем папку с архивом и выделяем архивный файл
                if os.name == 'nt':  # Windows
                    subprocess.Popen(f'explorer /select,"{archive_path}"')
                    self.status_var.set(f"Открыт архив: {os.path.basename(archive_path)}")
                elif os.name == 'posix':  # macOS, Linux
                    folder_path = os.path.dirname(archive_path)
                    subprocess.run(['open' if sys.platform == 'darwin' else 'xdg-open', folder_path])
                    self.status_var.set(f"Открыта папка с архивом: {folder_path}")
            except Exception as e:
                messagebox.showerror("Ошибка", f"Не удалось открыть расположение архива: {str(e)}")
        
        # Если это обычный файл на диске
