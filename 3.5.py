...
class FolderScannerApp:
    ...
    def on_hide_duplicates_changed(self):
        """Обработка изменения настройки скрытия дубликатов"""
    ...
    def create_menu(self):
        ...
        view_menu.add_checkbutton(label="Скрыть дубликаты", 
                                 variable=self.hide_duplicates_var,
                                 command=self.on_hide_duplicates_changed)
...
Кнопка 5 мыши - Повторить"""
    ...
    def toggle_hide_duplicates(self):
        """Переключить скрытие дубликатов"""
        self.settings["hide_duplicates"] = self.hide_duplicates_var.get()
        self.save_settings()
        self.refresh_tree()
    ...
    def refresh_tree(self):
        ...
        # Находим дубликаты если включена настройка
    ...
    def find_duplicates(self):
        ...
        for file_info in self.all_files:
            ...
            if filename in seen_names:
                self.duplicate_files.add(filename)
            else:
