...
class FolderScannerApp:
            ...
            if action['type'] == 'file_rename':
                ...
                for file_path in file_paths:
                    try:
                        ...
                        pass
                self.refresh_tree()
                self.status_var.set(f"Отменено удаление {len(file_paths)} файлов")
            ...
            if action['type'] == 'file_rename':
                ...
                for file_path in file_paths:
                    try:
                        ...
                        pass
                self.refresh_tree()
                self.status_var.set(f"Повторено удаление {len(file_paths)} файлов")
            ...
            for item in selected_items:
                ...
                if item_values and len(item_values) >= 1:
                    ...
                    if os.path.exists(file_path):
                        try:
                            # Создаем резервную копию для отмены
                            shutil.copy2(file_path, file_path + '.bak')
                            os.remove(file_path)
                            deleted_files.append(file_path)
                        except Exception as e:
                            messagebox.showerror("Ошибка", f"Не удалось удалить файл {file_path}: {str(e)}")
                self.tree.delete(item)
            
            if deleted_files:
                self.add_to_action_history('file_delete', {'file_paths': deleted_files})
    ...
    def adjust_search_height(self):
        """Автоматически настроить высоту поля поиска"""
    ...
    def toggle_virtual_folders(self):
        """Переключить состояние всех виртуальных папки"""
        ...
        if item:
            ...
            if item_values and len(item_values) >= 3:
                ...
                if item_type == 'virtual_folder':
                    # Получаем имя папки из текста элемента
                    item_text = self.tree.item(item, 'text')
                    folder_name = item_text.replace('📁 ', '').replace('⭐ ', '')
                    
                    # Открываем виртуальную папку в новом окне
                    self.open_virtual_folder_window(folder_name)
                elif item_type == 'file':
                    file_path = item_values[0]
    ...
    def open_virtual_folder_window(self, folder_name):
        """Открыть виртуальную папку в отдельном окне"""
        if folder_name not in self.virtual_folders:
            return
        
        # Создаем новое окно
        folder_window = tk.Toplevel(self.root)
        folder_window.title(f"Виртуальная папка: {folder_name}")
        folder_window.geometry("800x600")
        
        # Создаем список файлов
        listbox = tk.Listbox(folder_window, font=("Courier", 10))
        scrollbar = tk.Scrollbar(folder_window, orient=tk.VERTICAL, command=listbox.yview)
        ...
        # Добавляем атрибут для хранения путей
        listbox.file_paths = []
        ...
        def on_file_double_click(event):
            ...
            if selection:
                index = selection[0]
                if index < len(listbox.file_paths):
                    file_path = listbox.file_paths[index]
                    self.open_file(file_path)
        
        listbox.bind('<Double-1>', on_file_double_click)
    
    def extract_from_archive(self, archive_path, file_path):
        """Извлечь файл из архива во временную папку"""
        temp_file_path = None
        try:
    ...
    def open_file(self, file_path):
        """Открыть файл через стандартную ассоциацию Windows"""
        archive_path = None
        ...
        if archive_path and os.path.exists(archive_path):
            actual_file_path = self.extract_from_archive(archive_path, file_path)
        ...
        try:
            ...
            if messagebox.askyesno("Ошибка", 
                                   f"Не удалось открыть файл стандартным способом.\n"
                                   f"Хотите выбрать программу для открытия?"):
                self.open_file_with_windows_dialog(actual_file_path)
    
    def open_file_with_windows_dialog(self, file_path):
        """Открыть стандартный диалог Windows 'Открыть с помощью'"""
        try:
            # Используем ShellExecute с ключом 'openas' для вызова диалога "Открыть с помощью"
            ctypes.windll.shell32.ShellExecuteW(None, "openas", file_path, None, None, win32con.SW_SHOW)
            return True
        except Exception as e:
            try:
                ...
                messagebox.showerror("Ошибка", f"Не удалось открыть диалог 'Открыть с помощью':\n{str(e)}")
                return False
    
    def open_file_with(self):
        """Открыть файл с помощью стандартного диалога Windows"""
        selected_items = self.tree.selection()
        if not selected_items:
            return
        ...
        if len(item_values) >= 1:
            ...
            # Извлекаем файл из архива если нужно
            actual_file_path = file_path
            if archive_path and not os.path.exists(file_path):
                actual_file_path = self.extract_from_archive(archive_path, file_path)
            
            if actual_file_path and os.path.exists(actual_file_path):
                self.open_file_with_windows_dialog(actual_file_path)
                self.status_var.set(f"Открытие диалога для: {os.path.basename(file_path)}")
            else:
            ...
            if os.path.exists(file_path):
                ...
                try:
                    if os.name == 'nt':  # Windows
                        # Используем параметр /select для выделения файла
    ...
    def create_menu(self):
        ...
        special_menu.add_command(label="Сбросить настройки фильтров", command=self.reset_filters)
    ...
    def clear_search_cache(self):
        """Очистить кэш поиска"""
        self.file_content_cache = {}
        self.status_var.set("Кэш поиска очищен")
...
📂 Двойной клик по виртуальной папке открывает ее содержимое
