...
class FolderScannerApp:
        ...
        if item:
            ...
            if item_values and len(item_values) >= 3:
                ...
                if item_type == 'file':
                    ...
                    self.tree_menu.add_command(label="Добавить в папку", command=self.add_to_virtual_folder)
                    ...
                    self.tree_menu.add_command(label="Развернуть все", command=lambda: self.tree.item(item, open=True))
                    self.tree_menu.add_command(label="Свернуть все", command=lambda: self.tree.item(item, open=False))
    ...
    def add_to_virtual_folder(self):
        ...
        folder_menu = tk.Menu(self.tree, tearoff=0)
        folder_menu.add_command(label="Создать новую", command=lambda: self.create_new_virtual_folder(files_to_add))
        
        if self.virtual_folders:
            folder_menu.add_separator()
            for folder_name in self.virtual_folders.keys():
                folder_menu.add_command(label=folder_name, command=lambda f=folder_name: self.add_files_to_virtual_folder(f, files_to_add))
        
        folder_menu.post(self.root.winfo_pointerx(), self.root.winfo_pointery())
    
    def create_new_virtual_folder(self, files_to_add):
        """Создать новую виртуальную папку"""
    ...
    def add_files_to_virtual_folder(self, folder_name, files_to_add):
        ...
        self.save_settings()
        self.refresh_tree()
        self.status_var.set(f"Файлы добавлены в папку '{folder_name}'")
    ...
    def open_file_location(self):
        ...
        try:
            if os.name == 'nt':  # Windows
                # Правильное открытие расположения файла в проводнике
