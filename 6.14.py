...
class CustomTreeWidget(QTreeWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.drag_select_items = []  # Используем список вместо множества
        
        # Переменные для Ctrl+ЛКМ и Shift+ЛКМ
        self.last_selection = []  # Список последних выбранных элементов
    ...
    def mousePressEvent(self, event):
        if event.button() == Qt.LeftButton:
            ...
            self.last_selection = self.selectedItems()[:]  # Копируем список
    ...
    def mouseMoveEvent(self, event):
        if self.is_drag_selecting and event.buttons() & Qt.LeftButton:
            if self.drag_select_start:
                ...
                self.drag_select_items = new_selection[:]  # Копируем список
