...
class FolderScannerApp:
    def __init__(self, root):
        ...
        self.settings = {
            ...
            "sort_mode": "name_asc",  # name_asc, name_desc, date_asc, date_desc, size_asc, size_desc
    ...
    def setup_hotkeys(self):
        """Настройка горячих клавиш"""
        # Убираем конфликтующие привязки и добавляем правильные
    ...
    def undo_action(self):
        ...
        if self.current_action_index >= 0:
            ...
            if action['type'] == 'file_rename':
                ...
                self.save_settings()
                self.refresh_tree()
                action_text = "добавлены в" if was_favorite else "убраны из"
                self.status_var.set(f"Отменено: файлы {action_text} избранное")
    ...
    def redo_action(self):
        ...
        if self.current_action_index < len(self.action_history) - 1:
            ...
            if action['type'] == 'file_rename':
                ...
                self.save_settings()
                self.refresh_tree()
                action_text = "убраны из" if was_favorite else "добавлены в"
                self.status_var.set(f"Повторено: файлы {action_text} избранное")
    ...
    def create_widgets(self):
        ...
        # Привязки событий для дерева
        self.tree.bind('<Double-1>', self.on_tree_double_click)
        
        # Правильные привязки для выделения
    ...
    def on_tree_button_press(self, event):
        """Обработка нажатия кнопки мыши на дереве"""
        item = self.tree.identify_row(event.y)
        if item:
            ...
            self.is_dragging = False  # Пока не начали перетаскивать
    ...
    def on_tree_motion(self, event):
        ...
        if self.drag_start_item and self.drag_start_index is not None:
    ...
    def remove_file_from_virtual_folder(self, folder_name, file_path):
        ...
        if folder_name in self.virtual_folders:
            # Сохраняем состояние для истории действий
    ...
    def add_selected_to_virtual_folder(self, folder_name):
        ...
        for item in selected_items:
            ...
            if item_values and len(item_values) >= 3 and item_values[2] == 'file':
                ...
                # Проверяем, не находится ли файл уже в этой папке
                file_in_folder = False
                if folder_name in self.virtual_folders:
                    for existing_file in self.virtual_folders[folder_name]:
                        existing_path = existing_file['path'] if isinstance(existing_file, dict) else existing_file
        ...
        if not files_to_add:
            messagebox.showinfo("Информация", "Все выбранные файлы уже находятся в этой папке")
            return
    ...
    def toggle_virtual_folders(self):
        """Переключить состояние всех виртуальных папок (развернуть/свернуть)"""
    ...
    def add_files_to_virtual_folder(self, folder_name, files_to_add):
        ...
        # Убираем файлы из других папок при добавлении в новую
        for file_info in files_to_add:
            file_path = file_info['path']
            
            # Удаляем файл из всех других виртуальных папок
            for other_folder_name in list(self.virtual_folders.keys()):
                if other_folder_name != folder_name:
                    self.virtual_folders[other_folder_name] = [
                        f for f in self.virtual_folders[other_folder_name]
                        if (isinstance(f, dict) and f.get('path') != file_path) or f != file_path
                    ]
                    
                    # Удаляем пустые папки
                    if not self.virtual_folders[other_folder_name]:
                        del self.virtual_folders[other_folder_name]
                        if other_folder_name in self.settings.get("favorites", []):
                            self.settings["favorites"].remove(other_folder_name)
        ...
        if item:
            ...
            if item_values and len(item_values) >= 3:
                ...
                if item_type == 'virtual_folder':
                    # Переключаем состояние развертывания виртуальной папки (без задержки)
    ...
    def open_file(self, file_path):
        """Открыть файл с текстурами для моделей"""
        if not os.path.exists(file_path):
            messagebox.showerror("Ошибка", f"Файл не найден: {file_path}")
            return
        
        try:
            # Для моделей Blockbench пытаемся найти и загрузить текстуры
            if file_path.lower().endswith(('.json', '.bbmodel')):
                self.open_model_with_textures(file_path)
            else:
                # Для остальных файлов стандартное открытие
    ...
    def open_model_with_textures(self, model_path):
        """Открыть модель с автоматической загрузкой текстур"""
        try:
            # Читаем JSON файл модели
            with open(model_path, 'r', encoding='utf-8') as f:
                model_data = json.load(f)
            
            # Ищем текстуры в модели
            textures = {}
            if 'textures' in model_data:
                textures = model_data['textures']
            
            # Если есть текстуры и включена автозагрузка
            if textures and self.settings.get("auto_load_textures", True):
                # Создаем временную папку для текстур
                temp_dir = tempfile.mkdtemp()
                texture_paths = []
                
                # Ищем текстуры в той же папке что и модель
                model_dir = os.path.dirname(model_path)
                
                for texture_key, texture_value in textures.items():
                    if isinstance(texture_value, str):
                        # Пробуем разные расширения текстур
                        texture_name = texture_value
                        if ':' in texture_name:
                            texture_name = texture_name.split(':')[-1]
                        
                        # Ищем файл текстуры
                        for ext in ['.png', '.jpg', '.jpeg', '.tga']:
                            texture_path = os.path.join(model_dir, texture_name + ext)
                            if os.path.exists(texture_path):
                                # Копируем текстуру во временную папку
                                temp_texture_path = os.path.join(temp_dir, texture_name + ext)
                                shutil.copy2(texture_path, temp_texture_path)
                                texture_paths.append(temp_texture_path)
                                break
                
                # Если нашли текстуры, открываем модель в Blockbench
                if texture_paths and self.settings.get("blockbench_path"):
                    subprocess.Popen([self.settings["blockbench_path"], model_path])
                    self.status_var.set(f"Открытие модели с текстурами: {os.path.basename(model_path)}")
                else:
                    # Иначе стандартное открытие
                    if os.name == 'nt':
                        os.startfile(model_path)
                    elif os.name == 'posix':
                        subprocess.run(['open' if sys.platform == 'darwin' else 'xdg-open', model_path])
                    self.status_var.set(f"Открытие модели: {os.path.basename(model_path)}")
            else:
                # Стандартное открытие если нет текстур или отключена автозагрузка
                if os.name == 'nt':
                    os.startfile(model_path)
                elif os.name == 'posix':
                    subprocess.run(['open' if sys.platform == 'darwin' else 'xdg-open', model_path])
            ...
            # В случае ошибки открываем стандартным способом
            if os.name == 'nt':
                os.startfile(model_path)
            elif os.name == 'posix':
                subprocess.run(['open' if sys.platform == 'darwin' else 'xdg-open', model_path])
            self.status_var.set(f"Открытие файла: {os.path.basename(model_path)}")
    ...
    def open_file_location(self):
        ...
        try:
            if os.name == 'nt':  # Windows
                # Правильное открытие расположения файла в проводнике с выделением файла
                subprocess.Popen(f'explorer /select,"{file_path}"')
            elif os.name == 'posix':  # macOS, Linux
                folder_path = os.path.dirname(file_path)
                subprocess.run(['open' if sys.platform == 'darwin' else 'xdg-open', folder_path])
            
            self.status_var.set(f"Открыта папка с файлом: {os.path.dirname(file_path)}")
        except Exception as e:
            messagebox.showerror("Ошибка", f"Не удалось открыть расположение: {str(e)}")
    
    def open_file_with(self):
        """Открыть файл с помощью выбранной программы (диалог Windows)"""
        selected_items = self.tree.selection()
        if not selected_items:
            return
        ...
        try:
            if os.name == 'nt':  # Windows
                # Используем Windows API для вызова диалога "Открыть с помощью"
                ...
                if result <= 32:
                    raise Exception(f"Ошибка ShellExecute: {result}")
                    
            elif os.name == 'posix':  # macOS, Linux
                subprocess.run(['open' if sys.platform == 'darwin' else 'xdg-open', file_path])
            
            self.status_var.set(f"Открытие файла с помощью...")
        except Exception as e:
            messagebox.showerror("Ошибка", f"Не удалось открыть файл: {str(e)}")
    ...
    def toggle_favorite(self, update_immediately=False):
        ...
        if changed_files:
            ...
            if update_immediately:
                self.refresh_tree()
                action = "убраны из" if was_favorite else "добавлены в"
                self.status_var.set(f"Файлы {action} избранное")
    ...
    def refresh_tree(self):
        ...
        for item in self.tree.get_children():
            if self.tree.item(item, 'open'):
                ...
                expanded_folders.add(item_text)
        ...
        self.add_virtual_folders_to_tree(filtered_files, expanded_folders)
        
        # Добавляем файлы
        if self.settings.get("hide_duplicates", False):
            self.add_files_with_duplicates_grouping(filtered_files, expanded_folders)
        else:
    ...
    def add_virtual_folders_to_tree(self, filtered_files, expanded_folders):
    ...
    def add_files_with_duplicates_grouping(self, files, expanded_folders):
        ...
        for filename, file_list in duplicate_groups:
            ...
            # Восстанавливаем состояние развертывания
            if filename in expanded_folders:
        ...
        for root, dirs, files in os.walk(folder_path):
            ...
            for file in files:
                ...
                if ext:
                    ...
                    self.available_extensions.add("(без расширения)")
                    self.extension_data["(без расширения)"].append(file_info)
                ...
                for file_info in zip_ref.infolist():
                    if not file_info.is_dir():
                        ...
                        if ext:
                            ...
                            # Добавляем файлы без расширения в отдельную категорию
                            self.available_extensions.add("(без расширения)")
                            self.extension_data["(без расширения)"].append(file_data)
...
Версия: 5.1
Создатель: Akami_bl
Обратная связь: alami.bl@gmail.com
...
- Управление виртуальными папками
- Отмена/повтор действий
- Выделение файлов перетаскиванием
- Горячие клавиши"""
