...
class FolderScannerApp:
    ...
    def initialize_tree_bindings(self):
        ...
        if hasattr(self, 'tree') and self.tree.winfo_exists():
            ...
            # Привязка одинарного клика для открытия виртуальных папок
            self.tree.bind('<ButtonRelease-1>', self.on_tree_single_click)
    
    def on_tree_single_click(self, event):
        """Обработка одинарного клика для открытия/закрытия виртуальных папок"""
        if self.is_dragging:
            self.is_dragging = False
            return
        ...
        if item:
            ...
            if item_values and len(item_values) >= 3:
                ...
                # Для виртуальных папок - переключаем состояние по одинарному клику
                if item_type == 'virtual_folder' and self.tree.get_children(item):
    ...
    def undo_action(self):
        ...
        if self.current_action_index >= 0:
            ...
            if action['type'] == 'file_rename':
                ...
                if deleted_count > 0:
                    self.scan_selected()
                    self.status_var.set(f"Восстановлено файлов: {deleted_count}")
    ...
    def delete_selected(self):
        ...
        # Собираем информацию о файлах для истории
        files_to_delete = []
        ...
        if messagebox.askyesno("Подтверждение", f"Удалить {len(files_to_delete)} выбранных файлов?"):
            ...
            for file_info in files_to_delete:
                try:
                    # Перемещаем в корзину с сохранением пути для восстановления
                    trash_path = self.move_to_trash(file_info['original_path'])
                    if trash_path:
    ...
    def move_to_trash(self, file_path):
        ...
        try:
            # Для Windows используем winshell
            import winshell
            import send2trash
            ...
            # Получаем путь в корзине (не всегда возможно в Windows)
            # Возвращаем оригинальный путь как идентификатор
            return file_path
        except ImportError:
            # Если нет send2trash, пробуем использовать winshell
            try:
                import winshell
                winshell.delete_file(file_path, no_confirm=True)
                return file_path
            except:
                # Если не удалось переместить в корзину, удаляем навсегда
                os.remove(file_path)
                return file_path
        except Exception as e:
            # Если произошла ошибка, удаляем навсегда
            os.remove(file_path)
            return file_path
    ...
    def open_file_location(self):
        ...
        if len(item_values) >= 1:
            ...
            if os.path.exists(file_path):
                ...
                try:
                    if os.name == 'nt':  # Windows
                        # Команда для открытия проводника с выделенным файлом
                        subprocess.run(['explorer', '/select,', file_path])
                    elif sys.platform == 'darwin':  # macOS
                        subprocess.run(['open', '-R', file_path])
                    elif sys.platform.startswith('linux'):  # Linux
                        # Попробуем использовать xdg-open с папкой
                        subprocess.run(['xdg-open', folder_path])
                    
                    self.status_var.set(f"Открыта папка с выделенным файлом: {file_name}")
                except Exception as e:
                    # Резервный вариант - просто открыть папку
                    try:
                        if os.name == 'nt':
            ...
            try:
                ...
                if os.name == 'nt':  # Windows
                    try:
                        ...
                        subprocess.run(['rundll32.exe', 'shell32.dll,OpenAs_RunDLL', file_to_open])
                    except Exception as e:
                        # Резервный вариант - открыть программой по умолчанию
    ...
    def create_menu(self):
        ...
        # Добавляем подменю "Спец настройки"
        ...
        special_menu.add_checkbutton(label="Автозагрузка текстур", 
