...
class CrashReporter:
    """Класс для сбора и сохранения отчётов об ошибках"""
    ...
    def report_error(self, error_type, error_message, stack_trace=None, additional_info=None):
        ...
        try:
            ...
            # Оставляем только последние 50 отчётов
    ...
    def get_recent_crashes(self, count=10):
        """Получить последние отчёты об ошибках"""
        try:
            if os.path.exists(self.crash_file):
                with open(self.crash_file, 'r', encoding='utf-8') as f:
                    data = json.load(f)
...
class AppSettings:
    ...
    scan_history: List[str] = field(default_factory=list)  # Восстановлена история сканирования
    file_history: List[str] = field(default_factory=list)  # История открытых файлов
    ...
    # Панели инструментов - храним как словари для совместимости с JSON
    ...
    def to_dict(self):
        ...
        for key, value in self.__dict__.items():
            if key == 'toolbar_configs':
                # Убеждаемся, что все конфиги - словари
                result[key] = {}
...
class SettingsManager:
    ...
    def __init__(self, app_name="Sofil"):
        ...
        # Инициализируем репортер ошибок после загрузки папки данных
    ...
    def _migrate_v1(self, old_data):
        ...
        # Инициализируем историю файлов если её нет
        if 'file_history' not in migrated.__dict__:
            migrated.file_history = []
        
        # Инициализируем конфиги панелей инструментов как словари
    ...
    def report_error(self, error_type, error_message, additional_info=None):
        """Сообщить об ошибке через crash reporter"""
    ...
    def add_to_scan_history(self, path):
        """Добавить путь в историю сканирования"""
        if not path:
            return
    ...
    def add_to_file_history(self, path):
        """Добавить файл в историю открытых файлов"""
        if not path:
            return
...
class ToolbarEditorDialog(QDialog):
    ...
    def __init__(self, parent=None, settings_manager=None):
        ...
        # Создаем глубокую копию конфигураций как словарей
    ...
    def _add_custom_mode(self):
        ...
        if ok and name:
            ...
            # Создаем новый режим как словарь
            self.toolbar_configs[name] = {
                'mode': 'custom',
                'items': [],
                'show_toolbar': True,
    ...
    def get_result(self):
        ...
        return {
            ...
            'configs': self.toolbar_configs  # Уже словари
        }
...
class CustomToolBar(QToolBar):
    ...
    def apply_config(self, config):
        ...
        # Обрабатываем и словари, и объекты
        if isinstance(config, dict):
            # Преобразуем словарь в объект для удобства
            ...
            # Если ничего не подходит, создаем конфиг по умолчанию
            self.config = ToolbarConfig()
        ...
        try:
            if self.config.mode == 'full':
                ...
                # По умолчанию - минимальная панель
                self._build_minimal_toolbar()
        except Exception as e:
            print(f"Ошибка при построении панели инструментов: {e}")
            # В случае ошибки показываем минимальную панель
            self._build_minimal_toolbar()
    ...
    def _build_full_toolbar(self):
        ...
        if hasattr(self.parent_app, 'folder_combo') and self.parent_app.folder_combo:
            self.items['folder_combo'] = self.parent_app.folder_combo
            self.addWidget(self.parent_app.folder_combo)
        if hasattr(self.parent_app, 'browse_folder_btn') and self.parent_app.browse_folder_btn:
            self.addWidget(self.parent_app.browse_folder_btn)
        if hasattr(self.parent_app, 'browse_archive_btn') and self.parent_app.browse_archive_btn:
            self.addWidget(self.parent_app.browse_archive_btn)
        
        self.addSeparator()
        
        # Отмена/повтор
        if hasattr(self.parent_app, 'undo_btn') and self.parent_app.undo_btn:
            self.items['undo_btn'] = self.parent_app.undo_btn
            self.addWidget(self.parent_app.undo_btn)
        if hasattr(self.parent_app, 'redo_btn') and self.parent_app.redo_btn:
        ...
        if hasattr(self.parent_app, 'search_mode_btn') and self.parent_app.search_mode_btn:
            self.items['search_mode_btn'] = self.parent_app.search_mode_btn
            self.addWidget(self.parent_app.search_mode_btn)
        if hasattr(self.parent_app, 'multicriteria_btn') and self.parent_app.multicriteria_btn:
            self.items['multicriteria_btn'] = self.parent_app.multicriteria_btn
            self.addWidget(self.parent_app.multicriteria_btn)
        if hasattr(self.parent_app, 'search_input') and self.parent_app.search_input:
        ...
        if hasattr(self.parent_app, 'hide_duplicates_cb') and self.parent_app.hide_duplicates_cb:
        ...
        if hasattr(self.parent_app, 'sort_btn') and self.parent_app.sort_btn:
            self.items['sort_btn'] = self.parent_app.sort_btn
            self.addWidget(self.parent_app.sort_btn)
        if hasattr(self.parent_app, 'favorites_btn') and self.parent_app.favorites_btn:
        ...
        # Сканирование
        if hasattr(self.parent_app, 'scan_btn') and self.parent_app.scan_btn:
...
class FolderScannerApp(QMainWindow):
    ...
    def init_ui(self):
        ...
        if current_mode in configs:
            # Передаем конфиг как есть (это словарь)
    ...
    def create_menu(self):
        ...
        # Новое меню для отчётов об ошибках
    ...
    def show_scan_history(self):
        ...
        layout.addWidget(QLabel("Дважды кликните для загрузки:"))
    ...
    def show_file_history(self):
        ...
        layout.addWidget(QLabel("Дважды кликните для открытия:"))
        layout.addWidget(list_widget)
...
📜 В истории файлов: {len(self.settings_manager.settings.file_history)}"""
    ...
    def show_tree_context_menu(self, position):
        ...
        if item_type == 'file':
            ...
            # Используем новый метод open_with_dialog
    ...
    def open_file_default(self, file_path):
        ...
        try:
            ...
            if system == "Windows":
                # Windows: используем os.startfile для открытия программой по умолчанию
    ...
    def open_file_with_dialog(self, file_path):
        """Открыть диалог 'Открыть с помощью' для выбора программы"""
        if not os.path.exists(file_path):
            QMessageBox.warning(self, "Предупреждение", f"Файл не существует:\n{file_path}")
            return
        ...
        try:
            if system == "Windows":
                # Windows: используем rundll32 для вызова диалога "Открыть с помощью"
                try:
                    subprocess.run(
                    ...
                    # Если rundll32 не сработал, пробуем открыть стандартным способом
                    os.startfile(os.path.normpath(file_path))
                    
            elif system == "Darwin":  # macOS
                # macOS: открываем в Finder для выбора приложения
                subprocess.run(["open", "-a", "Finder", file_path], check=True)
                
            else:  # Linux
                # Linux: нет стандартного диалога, предлагаем открыть стандартной программой
                reply = QMessageBox.question(
                    self, 
                    "Открытие файла",
            ...
            # В случае ошибки пробуем открыть стандартным способом
            try:
                self.open_file_default(file_path)
            except:
                QMessageBox.critical(
    ...
    def on_file_double_click(self, item, column):
        ...
        if item_type == 'file':
            file_path = item.text(3)
            # Используем метод open_file_default
            self.open_file_default(file_path)
        elif item_type == 'folder':
