...
class FolderScannerApp:
    def __init__(self, root):
        ...
        self.hide_bb_children_var = tk.BooleanVar(value=True)
        ...
        self.settings = {
            ...
            "favorites": [],
            "last_search": "",  # Убрали сохранение поиска
            "last_extension": "",
            "hide_blockbench_children": True,
            "virtual_folders": {},
            "search_mode": "name",
            "auto_load_textures": False  # Новая настройка
        }
        ...
        # Словарь для отслеживания состояния раскрытия виртуальных папок
        self.virtual_folders_expanded_state = {}
        
        # Загружаем настройки
        self.load_settings()
        
        # Обновляем значение переменной из настроек
        self.hide_bb_children_var.set(self.settings.get("hide_blockbench_children", True))
    ...
    def initialize_tree_bindings(self):
        ...
        if hasattr(self, 'tree') and self.tree.winfo_exists():
            ...
            # Одинарный клик для сворачивания/разворачивания виртуальных папок
            self.tree.bind('<ButtonRelease-1>', self.on_tree_single_click)
    
    def on_tree_single_click(self, event):
        """Обработка одинарного клика для виртуальных папок"""
        ...
        if item:
            ...
            if len(item_values) >= 3:
                ...
                if item_type == 'virtual_folder':
                    ...
                    # Сохраняем состояние
                    self.virtual_folders_expanded_state[folder_name] = not current_state
    ...
    def undo_action(self):
        ...
        if self.current_action_index >= 0:
            ...
            if action['type'] == 'file_rename':
                ...
                if os.path.exists(new_path) and not os.path.exists(old_path):
                    try:
                        ...
                        # Обновляем состояние сразу после отмены
                        self.update_file_in_list(old_path, new_path, action_type='undo_rename')
                        self.refresh_tree_maintain_state()
                ...
                for file_info in action['data']['files']:
                    try:
                        if os.path.exists(file_info['temp_path']):
                            ...
                            os.rename(file_info['temp_path'], file_info['path'])
                        else:
                            with open(file_info['path'], 'wb') as f:
                                f.write(file_info['content'])
                        
                        # Добавляем файл обратно в список
                        self.add_file_back_to_list(file_info)
                        restored_count += 1
                    except Exception as e:
                        messagebox.showerror("Ошибка", f"Не удалось восстановить файл: {str(e)}")
                
                self.refresh_tree_maintain_state()
                self.status_var.set(f"Отменено удаление файлов: восстановлено {restored_count}")
                ...
                # Обновляем состояние файлов в основном списке
            ...
            elif action['type'] == 'virtual_folder_remove_file':
                folder_name = action['data']['folder_name']
                old_files = action['data']['old_files']
                self.virtual_folders[folder_name] = old_files
                self.save_settings()
                self.refresh_tree_maintain_state()
                self.status_var.set(f"Отменено удаление файла из папки '{folder_name}'")
            ...
            if action['type'] == 'file_rename':
                ...
                if os.path.exists(old_path) and not os.path.exists(new_path):
                    try:
                        ...
                        # Обновляем состояние сразу после повтора
                        self.update_file_in_list(new_path, old_path, action_type='redo_rename')
                        self.refresh_tree_maintain_state()
                ...
                for file_info in action['data']['files']:
                    try:
                        if os.path.exists(file_info['path']):
                            ...
                            self.remove_file_from_list(file_info['path'])
                            deleted_count += 1
                    except Exception as e:
                        messagebox.showerror("Ошибка", f"Не удалось удалить файл: {str(e)}")
                
                self.refresh_tree_maintain_state()
                self.status_var.set(f"Повторено удаление файлов: удалено {deleted_count}")
                ...
                if folder_name in self.virtual_folders:
                    # Удаляем файл из виртуальной папки
                    ...
                    self.save_settings()
                    self.refresh_tree_maintain_state()
                    self.status_var.set(f"Повторено удаление файла из папки '{folder_name}'")
            
            self.is_undo_redo_in_progress = False
            self.update_undo_redo_buttons()
    
    def update_file_in_list(self, target_path, source_path, action_type='rename'):
        """Обновить информацию о файле в основном списке"""
        for file_info in self.all_files:
            if file_info['path'] == source_path:
                file_info['path'] = target_path
                file_info['name'] = os.path.basename(target_path)
                
                # Обновляем избранное
                favorites = self.settings.get("favorites", [])
                if source_path in favorites:
                    favorites.remove(source_path)
                    favorites.append(target_path)
                    self.settings["favorites"] = favorites
                    file_info['is_favorite'] = True
                
                # Обновляем виртуальные папки
                for folder_name, files in self.virtual_folders.items():
                    for i, file_item in enumerate(files):
                        if isinstance(file_item, dict) and file_item.get('path') == source_path:
                            file_item['path'] = target_path
                            file_item['name'] = os.path.basename(target_path)
                        elif file_item == source_path:
                            files[i] = target_path
                
                self.save_settings()
                break
    
    def add_file_back_to_list(self, file_info):
        """Добавить файл обратно в основной список"""
        ...
        if not existing:
            ...
            new_file_info = {
                ...
                'path': file_info['path'],
                'relative_path': os.path.relpath(file_info['path'], self.folder_var.get()) if self.folder_var.get() and os.path.exists(self.folder_var.get()) else file_info['path'],
            ...
            self.all_files.append(new_file_info)
    
    def remove_file_from_list(self, file_path):
        """Удалить файл из основного списка"""
        self.all_files = [f for f in self.all_files if f['path'] != file_path]
    
    def refresh_tree_maintain_state(self):
        """Обновить дерево с сохранением состояния раскрытия виртуальных папок"""
        # Сохраняем текущее состояние раскрытия
        self.save_virtual_folders_state()
        
        # Обновляем дерево
        self.refresh_tree()
        
        # Восстанавливаем состояние раскрытия
        self.restore_virtual_folders_state()
    
    def save_virtual_folders_state(self):
        """Сохранить состояние раскрытия виртуальных папок"""
        for item in self.tree.get_children():
            item_values = self.tree.item(item, 'values')
            if len(item_values) >= 3 and item_values[2] == 'virtual_folder':
                folder_name = self.tree.item(item, 'text')
                if folder_name.startswith("⭐ "):
                    folder_name = folder_name[2:]
                elif folder_name.startswith("📁 "):
                ...
                self.virtual_folders_expanded_state[folder_name] = self.tree.item(item, 'open')
    
    def restore_virtual_folders_state(self):
        """Восстановить состояние раскрытия виртуальных папок"""
        for item in self.tree.get_children():
            item_values = self.tree.item(item, 'values')
            if len(item_values) >= 3 and item_values[2] == 'virtual_folder':
                folder_name = self.tree.item(item, 'text')
                ...
                if folder_name in self.virtual_folders_expanded_state:
                    self.tree.item(item, open=self.virtual_folders_expanded_state[folder_name])
                ...
                if len(item_values) >= 3 and item_values[2] == 'file':
                    ...
                    if os.path.exists(file_path):
                        try:
                            ...
                            messagebox.showerror("Ошибка", f"Не удалось прочитать файл {file_path}: {str(e)}")
                            continue
            ...
            if files_to_delete:
                ...
                for file_info in files_to_delete:
                    try:
                        ...
                        messagebox.showerror("Ошибка", f"Не удалось удалить файл {file_info['path']}: {str(e)}")
                
                self.refresh_tree_maintain_state()
                self.status_var.set(f"Удалено файлов: {len(files_to_delete)}")
        ...
        try:
            ...
            if os.path.exists(settings_path):
                with open(settings_path, 'r', encoding='utf-8') as f:
                    ...
                    # Сохраняем data_folder если он уже был установлен
                    ...
                    if "hide_blockbench_children" in loaded_settings:
                        self.settings["hide_blockbench_children"] = loaded_settings["hide_blockbench_children"]
                        
                    if "auto_load_textures" in loaded_settings:
                        self.settings["auto_load_textures"] = loaded_settings["auto_load_textures"]
    ...
    def save_settings(self):
        ...
        try:
            ...
            # Обновляем текущие настройки (НЕ сохраняем поиск)
            ...
            if hasattr(self, 'auto_load_textures_var'):
                self.settings["auto_load_textures"] = self.auto_load_textures_var.get()
    ...
    def create_widgets(self):
        ...
        self.search_var = tk.StringVar(value="")  # Всегда пустая строка
        ...
        # Группировка дубликатов (остается на панели инструментов)
        ...
        # Установка начальной папки если есть
    ...
    def show_tree_context_menu(self, event):
        ...
        if item:
            ...
            if item_values and len(item_values) >= 3:
                ...
                if item_type == 'file':
                    # Добавляем пункт "Удалить" в начало
                    self.tree_menu.add_command(label="Удалить", command=self.delete_selected)
                    ...
                    if in_virtual_folders:
                        ...
                        if len(in_virtual_folders) == 1:
                            self.tree_menu.add_command(label="Убрать из папки", 
                                                     command=lambda f=in_virtual_folders[0], p=file_path: self.remove_file_from_virtual_folder(f, p))
                        else:
    ...
    def rename_virtual_folder(self, folder_name):
        ...
        if new_name and new_name != folder_name:
            ...
            # Сохраняем состояние раскрытия для нового имени
            if folder_name in self.virtual_folders_expanded_state:
                self.virtual_folders_expanded_state[new_name] = self.virtual_folders_expanded_state.pop(folder_name)
    ...
    def delete_virtual_folder(self, folder_name):
        ...
        if messagebox.askyesno("Подтверждение", f"Вы уверены, что хотите удалить папку '{folder_name}'?"):
            if folder_name in self.virtual_folders:
                ...
                if folder_name in self.virtual_folders_expanded_state:
                    del self.virtual_folders_expanded_state[folder_name]
    ...
    def on_tree_double_click(self, event):
        ...
        if item:
            ...
            if item_values and len(item_values) >= 3:
                ...
                if item_type == 'file':
                    file_path = item_values[0]
                    self.open_file_with_dialog(file_path)
                elif item_type == 'folder':
                    current_state = self.tree.item(item, 'open')
                    self.tree.item(item, open=not current_state)
    
    def open_virtual_folder(self, folder_name):
        """Открыть виртуальную папку в проводнике - теперь не вызывается при двойном клике"""
        if folder_name in self.virtual_folders:
            files = self.virtual_folders[folder_name]
            if files:
                ...
                if os.path.exists(file_path):
                    ...
                    self.open_folder_with_selected_file(folder_path, file_path)
                else:
                    messagebox.showinfo("Информация", "Файлы виртуальной папки недоступны")
            else:
                messagebox.showinfo("Информация", "Виртуальная папка пуста")
    
    def open_file(self, file_path):
        """Открыть файл через стандартный диалог 'Открыть с помощью'"""
        # Всегда открываем через диалог "Открыть с помощью"
        self.open_file_with_dialog(file_path)
    
    def open_file_with_dialog(self, file_path):
        """Открыть файл через диалог 'Открыть с помощью' с использованием ShellExecute"""
        try:
            # Используем ShellExecute с командой "openas" для диалога "Открыть с помощью"
            shell32 = ctypes.windll.shell32
            
            # Сначала пробуем открыть через стандартный диалог Windows
            result = shell32.ShellExecuteW(
                0,                # hwnd
                "openas",         # lpOperation - "openas" открывает диалог "Открыть с помощью"
                file_path,        # lpFile
                None,             # lpParameters
                None,             # lpDirectory  
            ...
            if result <= 32:
                ...
                try:
                    # Используем команду "runas" для вызова диалога
                    result = shell32.ShellExecuteW(
                        0,
                        "runas",
                        "rundll32.exe",
                    ...
                    if result <= 32:
                        # Последний вариант - просто открыть
    ...
    def open_folder_with_selected_file(self, folder_path, file_path):
        ...
        try:
            ...
            self.status_var.set(f"Открыта папка с выделенным файлом: {os.path.basename(file_path)}")
        except Exception as e:
            try:
                os.startfile(folder_path)
    ...
    def open_file_with(self):
        """Открыть файл с помощью выбранной программы через системный диалог"""
        selected_items = self.tree.selection()
        if not selected_items:
            return
                            ...
                            with zipfile.ZipFile(archive_path, 'r') as zip_ref:
                                for zip_file in zip_ref.namelist():
                                    if zip_file.endswith(file_path) or os.path.basename(zip_file) == os.path.basename(file_path):
                                        with zip_ref.open(zip_file) as source, open(temp_file, 'wb') as target:
                            ...
                            with rarfile.RarFile(archive_path) as rar_ref:
                                for rar_file in rar_ref.namelist():
                                    if rar_file.endswith(file_path) or os.path.basename(rar_file) == os.path.basename(file_path):
                                        with rar_ref.open(rar_file) as source, open(temp_file, 'wb') as target:
                                            shutil.copyfileobj(source, target)
                                        file_path = temp_file
                                        break
                    except Exception as e:
                        messagebox.showerror("Ошибка", f"Не удалось извлечь файл: {str(e)}")
                        return
                else:
                    messagebox.showwarning("Предупреждение", "Файл не найден")
                    return
            
            # Открываем через диалог "Открыть с помощью"
            self.open_file_with_dialog(file_path)
    ...
    def rename_file(self):
        ...
        if new_name and new_name != old_name:
            ...
            try:
                ...
                # Обновляем информацию в виртуальных папках
                for folder_name, files in self.virtual_folders.items():
    ...
    def refresh_tree(self):
        ...
        if self.settings.get("hide_duplicates", False):
            self.add_files_with_duplicates_grouping_sorted(filtered_files)  # Измененный метод
        else:
    ...
    def add_virtual_folders_to_tree(self, filtered_files):
        ...
        for folder_name, files in self.virtual_folders.items():
            ...
            # Восстанавливаем состояние раскрытия
            if folder_name in self.virtual_folders_expanded_state:
                self.tree.item(folder_item, open=self.virtual_folders_expanded_state[folder_name])
    ...
    def add_files_with_duplicates_grouping_sorted(self, files):
        """Добавить файлы с группировкой дубликатов (группы в конце)"""
        ...
        # Сначала добавляем одиночные файлы
        ...
        # Затем добавляем группы дубликатов
        for filename, file_list in file_groups.items():
            if len(file_list) > 1:
    ...
    def create_menu(self):
        ...
        special_settings_menu = tk.Menu(settings_menu, tearoff=0)
        settings_menu.add_cascade(label="Спец настройки", menu=special_settings_menu)
        
        # Добавляем настройки в спец настройки
        special_settings_menu.add_checkbutton(label="Скрыть дочерние файлы Blockbench", 
                                              variable=self.hide_bb_children_var,
                                              command=self.on_hide_bb_children_changed)
        
        # Добавляем "Автозагрузка текстур" в спец настройки
        self.auto_load_textures_var = tk.BooleanVar(value=self.settings.get("auto_load_textures", False))
        special_settings_menu.add_checkbutton(label="Автозагрузка текстур", 
                                              variable=self.auto_load_textures_var,
                                              command=self.on_auto_load_textures_changed)
        ...
        # Группировка дубликатов перенесена в настройки
        settings_menu.add_checkbutton(label="Группировать дубликаты", 
                                     variable=self.hide_duplicates_var,
                                     command=self.on_hide_duplicates_changed)
        ...
        help_menu.add_command(label="Сочетания клавиш", command=self.show_hotkeys)
    ...
    def show_hotkeys(self):
        ...
        about_text = """Сочетания клавиш

Основные:
Ctrl+O — Открыть папку
Ctrl+Shift+O — Открыть архив
Ctrl+S — Сканировать выбранную папку
Ctrl+F — Добавить в избранное / убрать из избранного
Delete — Удалить выбранные файлы

Работа с файлами:
Ctrl+Z — Отменить действие
Ctrl+Y — Повторить действие
Ctrl+J — Создать виртуальную папку из выделенных файлов
...
Enter — Открыть файл"""
        ...
        messagebox.showinfo("Сочетания клавиш", about_text)
