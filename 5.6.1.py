...
class FolderScannerApp:
    ...
    def on_tree_single_click(self, event):
        """Обработка одинарного клика для открытия виртуальных папок"""
        if self.is_dragging:
            self.is_dragging = False
            return
        ...
        if item:
            ...
            if item_values and len(item_values) >= 3:
                ...
                if item_type == 'virtual_folder':
                    # Переключаем состояние развертывания виртуальной папки
    ...
    def setup_hotkeys(self):
        ...
        # Ctrl+G - переключить виртуальные папки
        self.root.bind('<Control-g>', lambda e: self.toggle_virtual_folders())
        ...
        if self.current_action_index >= 0:
            ...
            try:
                if action['type'] == 'file_rename':
                    ...
                    favorites = self.settings.get("favorites", [])
                    if folder_name in favorites and file_path not in favorites:
                        favorites.append(file_path)
                        self.settings["favorites"] = favorites
                ...
                messagebox.showerror("Ошибка отмены", f"Ошибка при отмене действия: {str(e)}")
            finally:
                self.is_undo_redo_in_progress = False
                self.update_undo_redo_buttons()
        ...
        if self.current_action_index < len(self.action_history) - 1:
            ...
            try:
                if action['type'] == 'file_rename':
                    ...
                    self.save_settings()
                    self.refresh_tree()
                    self.status_var.set(f"Повторено добавление {added_count} файлов в папку '{folder_name}'")
                ...
                messagebox.showerror("Ошибка повтора", f"Ошибка при повторе действия: {str(e)}")
            finally:
                self.is_undo_redo_in_progress = False
                self.update_undo_redo_buttons()
    ...
    def create_widgets(self):
        ...
        # Убираем иконки раскрытия для всех элементов
    ...
    def toggle_virtual_folder_favorite(self, folder_name):
        ...
        # Добавляем в историю действий
        self.add_to_action_history('virtual_folder_favorite_toggle', {
            'folder_name': folder_name,
    ...
    def open_file_with_dialog(self, file_path=None):
        """Открыть файл с помощью диалога выбора программы"""
        if not file_path:
            selected_items = self.tree.selection()
            if not selected_items:
                return
        ...
        if not os.path.exists(actual_file_path):
            messagebox.showwarning("Предупреждение", f"Файл не найден: {actual_file_path}")
            return
        
        try:
            # Для Windows используем ShellExecute с глаголом "openas"
            if os.name == 'nt':
                # Преобразуем путь в абсолютный и нормализуем
                abs_path = os.path.abspath(actual_file_path)
                
                # Используем ShellExecute для открытия диалога "Открыть с помощью"
                ...
                if result <= 32:
                    # Альтернативный метод через rundll32
                    subprocess.run(['rundll32.exe', 'shell32.dll,OpenAs_RunDLL', abs_path], shell=True)
                    
            elif sys.platform == 'darwin':  # macOS
                subprocess.run(['open', '-a', 'Finder', actual_file_path])
            else:  # Linux
                # Для Linux используем xdg-open
    ...
    def open_file_location(self):
        ...
        if len(item_values) >= 1:
            ...
            if archive_path and os.path.exists(archive_path):
                # Для архивных файлов открываем папку с архивом
                target_path = archive_path
                should_select = True
            elif os.path.exists(file_path):
                # Для обычных файлов открываем папку с файлом
            ...
            try:
                if os.name == 'nt':  # Windows
                    if should_select:
                        # Используем explorer с параметром /select, чтобы выделить файл/архив
                        subprocess.Popen(f'explorer /select,"{target_path}"', shell=True)
                        self.status_var.set(f"Открыта папка с выделением файла: {os.path.basename(target_path)}")
                    else:
    ...
    def create_menu(self):
        ...
        settings_menu.add_command(label="Автозагрузка текстур", 
                                 command=self.toggle_auto_load_textures)
        settings_menu.add_checkbutton(label="Скрыть дочерние файлы Blockbench", 
                                     variable=tk.BooleanVar(value=self.settings.get("hide_blockbench_children", True)),
    ...
    def show_hotkeys(self):
        """Показать список сочетаний клавиш"""
        hotkeys_text = """Сочетания клавиш:

Основные:
Ctrl+O - Открыть папку
Ctrl+Shift+O - Открыть архив
Ctrl+S - Сканировать выбранную папку/архив
Delete - Удалить выделенные элементы
Ctrl+F - Добавить/убрать из избранного

Работа с файлами:
Ctrl+Z - Отменить действие
Ctrl+Y - Повторить действие
Ctrl+J - Создать виртуальную папку из выделенного
Ctrl+G - Развернуть/свернуть все виртуальные папки

Навигация:
Shift+ЛКМ - Выделение диапазона файлов
ЛКМ с зажатием - Выделение перетаскиванием
Одинарный клик ЛКМ - Открыть/закрыть папку
Двойной клик ЛКМ - Открыть файл с помощью...
Правый клик - Контекстное меню

Для сброса выделения кликните на пустое место в дереве файлов."""
        
        messagebox.showinfo("Сочетания клавиш", hotkeys_text)
...
Сохраняет все настройки"""
    ...
    def toggle_auto_load_textures(self):
        ...
        self.save_settings()
        messagebox.showinfo("Настройки", f"Автозагрузка текстур: {'включена' if self.settings['auto_load_textures'] else 'отключена'}")
