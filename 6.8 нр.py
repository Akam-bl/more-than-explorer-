...
class ScanThread(QThread):
    ...
    def scan_real_folder(self, folder_path, all_files, extension_data, available_extensions, all_folders):
        ...
        try:
            ...
            self.error.emit(f"Ошибка при подсчете файлов: {str(e)}")
            return
            ...
            for file in files:
                ...
                try:
                    ...
                    if processed_files % 100 == 0:
                        ...
                        self.progress_update.emit(f"Сканирование... {processed_files}/{total_files} файлов", progress_percent)
                
                except Exception as e:
                    # Пропускаем файлы с ошибками и продолжаем сканирование
                    continue
                ...
                with zipfile.ZipFile(archive_path, 'r') as zip_ref:
                    ...
                    for i, file_info in enumerate(file_list):
                        ...
                        if not file_info.is_dir():
                            ...
                            if processed_files % 100 == 0:
                                progress_percent = int((processed_files / total_files) * 100) if total_files > 0 else 0
                                self.progress_update.emit(f"Сканирование архива... {processed_files}/{total_files} файлов", progress_percent)
                    
                    # Финальное обновление прогресса
                    if total_files > 0:
                        self.progress_update.emit(f"Архив просканирован: {total_files} файлов", 100)
            
            except Exception as e:
                self.error.emit(f"Ошибка при сканировании архива: {str(e)}")
...
class FolderScannerApp(QMainWindow):
    ...
    def scan_selected(self):
        ...
        self.progress_bar.setRange(0, 100)  # Устанавливаем диапазон 0-100
...
Версия: 6.8
Главный кодер: Akami_bl
Главный тестер: Akami_bl
Главный дизайнер: Akami_bl
Тестеры: yastno09
Дизайнеры: skivvelk, Ravensqw
