...
📜 В истории файлов: {len(self.settings_manager.settings.file_history)}"""
    ...
    • Отображение файлов по расширениям
    • Поддержка вложенных папок
    • Отображение размера и даты изменения файлов
    • Таймер сканирования
    ...
    • Поиск по дате (точная дата или период)
    • Поиск по размеру (>, <, =, диапазоны)
    • Мультипоиск с логическими операторами:
    ...
    📊 СОРТИРОВКА:
    • По имени (A-Z / Z-A)
    • По дате изменения (сначала новые / сначала старые)
    • По размеру (сначала большие / сначала маленькие)

    ⭐ ИЗБРАННОЕ:
    • Добавление файлов в избранное
    • Фильтр по избранным файлам
    • Быстрый доступ к избранным виртуальным папкам

    📁 ВИРТУАЛЬНЫЕ ПАПКИ:
    • Создание виртуальных папок для группировки файлов
    • Добавление файлов в виртуальные папки
    • Переименование виртуальных папок
    • Удаление виртуальных папок
    • Добавление виртуальных папок в избранное
    • Экспорт виртуальных папок в архивы

    📦 ПАКЕТНАЯ ОБРАБОТКА:
    • Упаковка файлов в архивы (ZIP, TAR, TAR.GZ, 7z, RAR)
    • Распаковка архивов
    • Сортировка файлов перед упаковкой (по имени, дате, размеру)
    • Гибкая система именования:
    • Шаблоны имен (с подстановкой {base})
    • 4 стиля нумерации (space_paren, dot, underscore, bracket)
    • Очередь шаблонов для точного сопоставления
    • Параллельное выполнение (1-16 потоков)
    • Обработка ошибок (продолжить / остановить)

    ✏️ MASS RENAME (POWER RENAME++):
    • Массовое переименование нескольких файлов
    • Поиск и замена в именах файлов
    • Поддержка регулярных выражений
    • Режимы регистра (с учетом / без учета / полное совпадение)
    • Добавление префиксов и суффиксов
    • Умная нумерация (старт, шаг, ширина)
    • Сохранение расширения (опционально)
    • Предпросмотр перед применением
    • Подсветка конфликтов имен

    ↩️ ИСТОРИЯ И ОТМЕНА:
    • История сканирования (до 20 последних путей)
    • История открытых файлов (до 50 последних)
    • Отмена действий (Ctrl+Z):
    • Переименование файлов
    • Удаление файлов
    • Добавление/удаление из избранного
    • Создание/удаление виртуальных папок
    • Добавление файлов в виртуальные папки
    • Повтор отмененных действий (Ctrl+Y)

    🎨 НАСТРОЙКИ ИНТЕРФЕЙСА:
    • Тёмная / светлая тема
    • Настраиваемая панель инструментов:
    • 3 предустановленных режима (полный, просмотр, минимальный)
    • Создание пользовательских режимов
    • Добавление/удаление элементов
    • Изменение порядка элементов
    • Гибкая компоновка окон (разделитель)

    🖱️ УПРАВЛЕНИЕ ФАЙЛАМИ:
    • Открытие файлов двойным кликом
    • Открытие расположения файла
    • Открытие с помощью...
    • Переименование отдельных файлов
    • Массовое переименование (Power Rename++)
    • Удаление файлов с возможностью отмены
    ...
    • Количество избранных файлов
    • Количество виртуальных папок

    💥 ОТЧЁТЫ ОБ ОШИБКАХ:
    • Автоматическое сохранение отчётов об ошибках
    • Просмотр последних ошибок
    • Очистка отчётов
    • Полный стек вызовов в отчёте
    ...
    • ZIP (встроенная поддержка)
    • TAR / TAR.GZ (встроенная поддержка)
    • 7z (через внешнюю утилиту 7z)
    • RAR (через внешнюю утилиту unrar/rar)

    💾 НАСТРОЙКИ ХРАНЕНИЯ:
    • Папка для данных приложения
    • Временная папка
    • Путь к UnRAR (для RAR архивов)
