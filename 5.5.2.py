...
class FolderScannerApp:
    ...
    def on_tree_single_click(self, event):
        ...
        if item:
            ...
            if item_values and len(item_values) >= 3:
                ...
                if item_type in ['virtual_folder', 'folder']:
            ...
            try:
                if action['type'] == 'file_rename':
                    ...
                    for file_info in files:
                        try:
                            if os.path.exists(file_info['trash_path']):
                    ...
                    for file_info in files:
                        try:
                            if os.path.exists(file_info['original_path']):
                                trash_path = self.move_to_trash(file_info['original_path'])
                                if trash_path:
                                    deleted_count += 1
                        except Exception:
                            continue
    ...
    def move_to_trash(self, file_path):
        ...
        try:
            # Используем send2trash для безопасного перемещения в корзину
            import send2trash
            send2trash.send2trash(file_path)
            return file_path
        except ImportError:
            try:
                # Альтернативный способ через winshell
                import winshell
    ...
    def open_file_location(self):
        ...
        if len(item_values) >= 1:
            ...
            if os.path.exists(file_path):
                try:
                    if os.name == 'nt':
                        # Правильный способ открыть проводник с выделенным файлом в Windows
                        subprocess.Popen(f'explorer /select,"{file_path}"', shell=True)
                    elif sys.platform == 'darwin':
                        subprocess.Popen(['open', '-R', file_path])
                    elif sys.platform.startswith('linux'):
                        # Для Linux используем xdg-open с директорией
    ...
    def open_file_with_dialog(self):
        ...
        if len(item_values) >= 1:
            ...
            try:
                ...
                # Открываем стандартный диалог "Открыть с помощью" Windows
                if os.name == 'nt':
                    try:
                        # Используем ShellExecute для вызова диалога "Открыть с помощью"
                        ...
                        subprocess.Popen(f'rundll32.exe shell32.dll,OpenAs_RunDLL "{file_to_open}"', shell=True)
                else:
                    # Для Linux/macOS открываем программой по умолчанию
