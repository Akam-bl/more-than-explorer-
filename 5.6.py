    ...
    def open_file_with_dialog(self, file_path=None):
        ...
        try:
            ...
            if not os.path.exists(actual_file_path):
                messagebox.showerror("Ошибка", f"Не удалось получить доступ к файлу: {file_path}")
                return
            
            # Используем Windows API для вызова диалога "Открыть с помощью"
            if os.name == 'nt':  # Windows
                try:
                    # Первый способ: через ShellExecute с глаголом "openas"
                    ...
                    if result <= 32:
                        # Второй способ: через команду "rundll32"
                        subprocess.run(['rundll32.exe', 'shell32.dll,OpenAs_RunDLL', actual_file_path])
                except Exception as e:
                    # Третий способ: через start
                    subprocess.run(['start', 'openas', actual_file_path], shell=True)
            else:  # Linux, macOS
                # Для Linux используем xdg-open
                if sys.platform == 'linux':
                    subprocess.run(['xdg-open', actual_file_path])
                # Для macOS используем open
                elif sys.platform == 'darwin':
                    subprocess.run(['open', actual_file_path])
    ...
    def open_file_location(self):
        ...
        if len(item_values) >= 1:
            ...
            if os.path.exists(file_path):
                # Открываем папку с выделением файла в проводнике Windows
                try:
                    if os.name == 'nt':  # Windows
                        # Используем explorer с параметром /select,
                        # чтобы выделить файл
