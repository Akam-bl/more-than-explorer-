            ...
            if item_values and len(item_values) >= 3:
                ...
                if item_type == 'file':
                    ...
                    if folder_name in self.settings.get("favorites", []):
                        self.tree_menu.add_command(label="Убрать из избранного", command=lambda: self.toggle_virtual_folder_favorite(folder_name))
                    else:
                        self.tree_menu.add_command(label="Добавить в избранное", command=lambda: self.toggle_virtual_folder_favorite(folder_name))
                    
                    self.tree_menu.add_command(label="Переименовать папку", command=lambda: self.rename_virtual_folder(folder_name))
                    self.tree_menu.add_command(label="Удалить папку", command=lambda: self.delete_virtual_folder(folder_name))
    ...
    def on_drag_motion(self, event):
        ...
        if self.drag_start_item:
            ...
            if current_item:
                ...
                try:
                    start_index = all_items.index(self.drag_start_item)
                    current_index = all_items.index(current_item)
                    
                    start_idx = min(start_index, current_index)
                    end_idx = max(start_index, current_index)
        ...
        if item:
            ...
            if item_values and len(item_values) >= 3:
                ...
                if item_type == 'virtual_folder':
                    ...
                    # Переключаем состояние развертывания обычной папки без задержки
                    current_state = self.tree.item(item, 'open')
                    self.tree.item(item, open=not current_state)
        ...
        for file_info in files:
            ...
            for folder_files in self.virtual_folders.values():
                for f in folder_files:
                    if isinstance(f, dict) and f.get('path') == file_info['path']:
