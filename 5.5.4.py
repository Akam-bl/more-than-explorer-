...
class FolderScannerApp:
    ...
    def on_tree_single_click(self, event):
        """Обработка одинарного клика для открытия/закрытия папок"""
        if self.is_dragging:
            self.is_dragging = False
            return
        ...
        if item:
            ...
            if item_values and len(item_values) >= 3:
                ...
                # Для виртуальных папок и обычных папок - переключаем состояние по одинарному клику
                if item_type in ['virtual_folder', 'folder'] and self.tree.get_children(item):
    ...
    def add_to_action_history(self, action_type, data):
        ...
        while len(self.action_history) > self.current_action_index + 1:
            ...
            try:
                if action['type'] == 'file_rename':
                    ...
                    if os.path.exists(new_path):
                        ...
                        self.update_file_path_in_data(old_path, new_path)
                        self.scan_selected()
                    ...
                    if new_name in self.virtual_folders:
                        ...
                        self.save_settings()
                        self.refresh_tree()
                        self.status_var.set(f"Отменено переименование папки: '{new_name}' -> '{old_name}'")
                
                elif action['type'] == 'file_delete':
                    deleted_files = action['data']['deleted_files']
                    self.status_var.set(f"Удаление отменено для {len(deleted_files)} файлов")
                    # Примечание: восстановление из корзины сложно реализовать
                    # Вместо этого просто обновляем список файлов
                    self.scan_selected()
                    ...
                    if folder_name in self.virtual_folders:
                        ...
                        for file_info in files_added:
                            self.virtual_folders[folder_name] = [
                                ...
                                if (isinstance(f, dict) and f.get('path') != file_info.get('path')) or f != file_info
                            ]
                        
                        self.save_settings()
                        self.refresh_tree()
                        self.status_var.set(f"Отменено добавление файлов в папку '{folder_name}'")
                    ...
                    if folder_name in self.virtual_folders:
                        # Восстанавливаем старый список файлов
                ...
                messagebox.showerror("Ошибка", f"Не удалось отменить действие: {str(e)}")
            ...
            try:
                if action['type'] == 'file_rename':
                    ...
                    if os.path.exists(old_path):
                        ...
                        self.update_file_path_in_data(new_path, old_path)
                        self.scan_selected()
                    ...
                    if old_name in self.virtual_folders:
                        ...
                        self.save_settings()
                        self.refresh_tree()
                        self.status_var.set(f"Повторено переименование папки: '{old_name}' -> '{new_name}'")
                
                elif action['type'] == 'file_delete':
                    files_to_delete = action['data']['files_to_delete']
                    deleted_count = 0
                    
                    for file_info in files_to_delete:
                        try:
                            if os.path.exists(file_info['original_path']):
                                self.move_to_trash(file_info['original_path'])
                                deleted_count += 1
                        except:
                            continue
                    ...
                    if deleted_count > 0:
                        self.scan_selected()
                        self.status_var.set(f"Удалено файлов: {deleted_count}")
                    ...
                    self.save_settings()
                    self.refresh_tree()
                    self.status_var.set(f"Повторено добавление файлов в папку '{folder_name}'")
                ...
                messagebox.showerror("Ошибка", f"Не удалось повторить действие: {str(e)}")
            
            finally:
                self.is_undo_redo_in_progress = False
                self.update_undo_redo_buttons()
    
    def update_file_path_in_data(self, new_path, old_path):
        """Обновить путь к файлу в данных"""
    ...
    def delete_selected(self):
        ...
        # Собираем реальные файлы для удаления (не папки)
        files_to_delete = []
        ...
        if messagebox.askyesno("Подтверждение", f"Удалить {len(files_to_delete)} выбранных файлов?"):
            deleted_files = []
            for file_info in files_to_delete:
                try:
                    if self.move_to_trash(file_info['original_path']):
                        deleted_files.append(file_info)
                except Exception as e:
                    messagebox.showerror("Ошибка", f"Не удалось удалить файл {file_info['name']}: {str(e)}")
            ...
            if deleted_files:
                ...
                self.scan_selected()
                self.status_var.set(f"Удалено файлов: {len(deleted_files)}")
    
    def move_to_trash(self, file_path):
        """Переместить файл в корзину Windows"""
        try:
            # Попробуем использовать send2trash если установлен
            try:
                from send2trash import send2trash
                send2trash(file_path)
                return True
            except ImportError:
                # Если send2trash не установлен, используем альтернативные методы
                if os.name == 'nt':  # Windows
                    # Используем Shell API для перемещения в корзину
                    shell32 = ctypes.windll.shell32
                    
                    # Конвертируем путь в формат Unicode
                    wc_file_path = ctypes.create_unicode_buffer(file_path)
                    ...
                    shf.hwnd = None
                    shf.wFunc = FO_DELETE
                    shf.pFrom = wc_file_path + '\0'  # Двойной null-terminated string
                    shf.pTo = None
                    ...
                    # Для Linux/macOS используем стандартное удаление
                    os.remove(file_path)
                    return True
        except Exception as e:
            print(f"Ошибка при удалении файла {file_path}: {e}")
            # В крайнем случае используем стандартное удаление
            try:
                os.remove(file_path)
                return True
            except:
    ...
    def create_widgets(self):
        ...
        self.undo_btn = ttk.Button(toolbar, text="↶ Отменить", command=self.undo_action, width=10, state=tk.DISABLED)
        self.undo_btn.pack(side=tk.LEFT, padx=(10, 5))
        
        self.redo_btn = ttk.Button(toolbar, text="↷ Повторить", command=self.redo_action, width=10, state=tk.DISABLED)
        ...
        self.favorites_btn = ttk.Button(toolbar, text="⭐ Избранное", command=self.toggle_favorites_filter)
        ...
        if item:
            ...
            if item_values and len(item_values) >= 3:
                ...
                if item_type == 'file':
                    ...
                    self.tree_menu.add_command(label="🔗 Открыть с помощью...", command=self.open_file_with_dialog)
                    self.tree_menu.add_command(label="✏️ Переименовать", command=self.rename_file)
                    self.tree_menu.add_command(label="🗑️ Удалить", command=self.delete_selected)
                    ...
                    if file_path in favorites:
                        self.tree_menu.add_command(label="⭐ Убрать из избранного", 
                                                  command=lambda: self.toggle_favorite(update_immediately=True))
                    else:
                    ...
                    add_to_folder_menu.add_command(label="📝 Создать новую", 
                                                  command=self.create_new_virtual_folder_from_context)
                    
                    if self.virtual_folders:
                        add_to_folder_menu.add_separator()
                        for folder_name in self.virtual_folders.keys():
                            add_to_folder_menu.add_command(label=f"📂 {folder_name}", 
                    ...
                    if in_virtual_folders:
                        ...
                        for folder_name in in_virtual_folders:
                            remove_menu.add_command(label=f"📂 {folder_name}", 
                    ...
                    self.tree_menu.add_command(label="✏️ Переименовать папку", 
                                             command=lambda: self.rename_virtual_folder(folder_name))
                    self.tree_menu.add_command(label="🗑️ Удалить папку", 
    ...
    def delete_virtual_folder(self):
        ...
        item_values = self.tree.item(item, 'values')
        if not item_values or len(item_values) < 3 or item_values[2] != 'virtual_folder':
            return
    ...
    def open_file_with_default(self, file_path):
        ...
        # Проверяем, находится ли файл в архиве
        archive_path = None
        for file_info in self.all_files:
        ...
        if archive_path and os.path.exists(archive_path):
            # Извлекаем файл из архива во временную папку
            ...
            try:
                ...
                messagebox.showerror("Ошибка", f"Не удалось извлечь файл из архива: {str(e)}")
                return
    ...
    def open_file_location(self):
        """Открыть расположение файла в проводнике и выделить файл"""
        selected_items = self.tree.selection()
        if not selected_items:
            return
        ...
        if len(item_values) >= 1:
            ...
            if os.path.exists(file_path):
                try:
                    if os.name == 'nt':
                        # Используем ShellExecute для открытия проводника с выделенным файлом
                    ...
                    messagebox.showerror("Ошибка", f"Не удалось открыть расположение файла: {str(e)}")
            else:
                messagebox.showwarning("Предупреждение", "Файл не существует")
    
    def open_file_with_dialog(self):
        """Открыть стандартный диалог Windows 'Открыть с помощью'"""
        selected_items = self.tree.selection()
        if not selected_items:
            return
        ...
        if len(item_values) >= 1:
            ...
            try:
                if os.name == 'nt':  # Windows
                    ...
                    if result <= 32:
                        # Если ShellExecute не сработал, пробуем альтернативный способ
                        subprocess.Popen(f'rundll32.exe shell32.dll,OpenAs_RunDLL "{file_path}"', shell=True)
                    
                    self.status_var.set(f"Открытие файла: {os.path.basename(file_path)}")
                else:  # Linux/macOS
                    # Для не-Windows систем открываем программой по умолчанию
    ...
    def create_menu(self):
        ...
        special_menu = tk.Menu(settings_menu, tearoff=0)
        settings_menu.add_cascade(label="Спец настройки", menu=special_menu)
        
        self.hide_blockbench_children_var = tk.BooleanVar(value=self.settings.get("hide_blockbench_children", True))
        special_menu.add_checkbutton(label="Скрыть дочерние файлы Blockbench", 
                                   variable=self.hide_blockbench_children_var,
                                   command=self.on_hide_blockbench_children_changed)
    ...
    def on_hide_blockbench_children_changed(self):
        """Обработка изменения настройки скрытия дочерних файлов Blockbench"""
        self.settings["hide_blockbench_children"] = self.hide_blockbench_children_var.get()
        self.save_settings()
        self.refresh_tree()
