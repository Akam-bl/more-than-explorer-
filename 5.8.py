...
class FolderScannerApp:
    ...
    def undo_action(self):
        ...
        if self.current_action_index >= 0:
            ...
            if action['type'] == 'file_rename':
                ...
                self.refresh_tree()
                self.status_var.set("Отменено удаление файлов")
    ...
    def redo_action(self):
        ...
        if self.current_action_index < len(self.action_history) - 1:
            ...
            if action['type'] == 'file_rename':
                ...
                self.refresh_tree()
                self.status_var.set("Повторено удаление файлов")
    ...
    def delete_selected(self):
        ...
        if messagebox.askyesno("Подтверждение удаления", 
                              f"Вы уверены, что хотите удалить {len(selected_items)} выбранных элементов?"):
            files_to_delete = []
            ...
            if os.path.exists(settings_path):
                with open(settings_path, 'r', encoding='utf-8') as f:
                    ...
                    if "last_search" in loaded_settings:
                        self.settings["last_search"] = loaded_settings["last_search"]
    ...
    def create_widgets(self):
        ...
        # Кнопка удаления на панели инструментов
        self.delete_btn = ttk.Button(toolbar, text="Удалить", command=self.delete_selected, state=tk.DISABLED)
        self.delete_btn.pack(side=tk.LEFT, padx=(10, 5))
        ...
        # Кнопка избранного (перенесена из меню)
    ...
    def on_tree_selection_change(self, event):
        ...
        selected_items = self.tree.selection()
        if selected_items:
            # Проверяем, есть ли среди выбранных элементов файлы
            has_files = False
            for item in selected_items:
                item_values = self.tree.item(item, 'values')
            ...
            # Включаем кнопку удаления только если есть файлы
            self.delete_btn.config(state=tk.NORMAL if has_files else tk.DISABLED)
        else:
        ...
        if item:
            ...
            if item_values and len(item_values) >= 3:
                ...
                if item_type == 'file':
                    ...
                    self.tree_menu.add_command(label="Открыть с помощью...", command=self.open_file_with_dialog_from_menu)
                    self.tree_menu.add_command(label="Переименовать", command=self.rename_file)
                    
                    # Добавляем пункт "Удалить" в контекстное меню
    ...
    def open_file_with_dialog_from_menu(self):
        """Открыть файл с помощью диалога выбора программы из контекстного меню"""
        selected_items = self.tree.selection()
        if not selected_items:
            return
        ...
        if item:
            ...
            if item_values and len(item_values) >= 3:
                ...
                if item_type == 'virtual_folder':
                    # Реализуем toggle-логику для виртуальной папки
    ...
    def open_virtual_folder(self, folder_name):
        """Открыть виртуальную папку - только toggle состояния"""
        if folder_name in self.virtual_folders:
            # Находим элемент в дереве
            for item in self.tree.get_children():
                if self.tree.item(item, 'text').replace('📁 ', '').replace('⭐ ', '') == folder_name:
                    current_state = self.tree.item(item, 'open')
                    self.tree.item(item, open=not current_state)
                    break
    ...
    def open_file_with_dialog(self, file_path):
        ...
        # Вызываем стандартный диалог "Открыть с помощью"
        try:
            # Используем ShellExecute с командой "openas" для вызова диалога
            shell32 = ctypes.windll.shell32
            result = shell32.ShellExecuteW(
                None,          # hwnd
                "openas",      # операция - открыть с помощью
                actual_file_path,  # файл
                None,          # параметры
                None,          # директория
                1              # SW_SHOWNORMAL
            )
            ...
            if result <= 32:
                ...
                os.startfile(actual_file_path)
            
            self.status_var.set(f"Открытие файла с помощью диалога: {os.path.basename(actual_file_path)}")
        except Exception as e:
            # Резервный способ
            os.startfile(actual_file_path)
            self.status_var.set(f"Открытие файла: {os.path.basename(actual_file_path)}")
    ...
    def refresh_tree(self):
        ...
        # Добавляем файлы с учетом новой логики сортировки (дубликаты ниже)
        self.add_files_with_special_sorting(filtered_files)
    ...
    def add_files_with_special_sorting(self, files):
        """Добавить файлы с специальной сортировкой (дубликаты ниже обычных файлов)"""
        if self.settings.get("hide_duplicates", False):
            self.add_files_with_duplicates_grouping_sorted(files)
        else:
            self.add_files_normal_sorted(files)
    
    def add_files_normal_sorted(self, files):
        """Добавить файлы в обычном режиме с правильной сортировкой"""
        for file_info in files:
    ...
    def add_files_with_duplicates_grouping_sorted(self, files):
        """Добавить файлы с группировкой дубликатов (дубликаты ниже обычных файлов)"""
    ...
    def create_menu(self):
        """Создание меню приложения с новой структурой"""
        ...
        # Специальные настройки как подменю
        special_settings_menu = tk.Menu(settings_menu, tearoff=0)
        settings_menu.add_cascade(label="Специальные настройки", menu=special_settings_menu)
        
        # Добавляем настройку группировки дубликатов в специальные настройки
        self.hide_duplicates_var = tk.BooleanVar(value=self.settings.get("hide_duplicates", False))
        special_settings_menu.add_checkbutton(label="Группировать дубликаты", 
                                             variable=self.hide_duplicates_var,
                                             command=self.on_hide_duplicates_changed)
        
        # Добавляем настройку скрытия дочерних файлов Blockbench
        ...
        view_menu.add_command(label="Сортировка", command=self.toggle_sorting)
        
        # Помощь с горячими клавишами
    ...
    def show_hotkeys(self):
        """Показать таблицу горячих клавиш"""
        hotkeys_info = """
Горячие клавиши Sofil

┌────────────────────────────┬─────────────────────┐
│ Действие                   │ Сочетание клавиш    │
├────────────────────────────┼─────────────────────┤
│ Открыть папку              │ Ctrl + O            │
│ Открыть архив              │ Ctrl + Shift + O    │
│ Сканировать                │ Ctrl + S            │
│ Удалить выбранное          │ Delete              │
│ В избранное                │ Ctrl + F            │
│ Отменить действие          │ Ctrl + Z            │
│ Повторить действие         │ Ctrl + Y            │
│ Создать папку из выделения │ Ctrl + J            │
└────────────────────────────┴─────────────────────┘

Управление мышью:
• Перетаскивание с зажатой левой кнопкой - выделение диапазона
• Shift + клик - выделение диапазона
• Двойной клик по файлу - открыть с помощью...
• Двойной клик по папке - развернуть/свернуть
• Правая кнопка - контекстное меню
"""
        
        # Создаем отдельное окно для горячих клавиш
        hotkeys_window = tk.Toplevel(self.root)
        hotkeys_window.title("Горячие клавиши")
        hotkeys_window.geometry("600x400")
        hotkeys_window.transient(self.root)
        hotkeys_window.grab_set()
        
        # Добавляем текстовое поле с прокруткой
        text_frame = ttk.Frame(hotkeys_window)
        text_frame.pack(fill=tk.BOTH, expand=True, padx=10, pady=10)
        
        text_widget = tk.Text(text_frame, wrap=tk.WORD, font=("Courier", 10))
        text_widget.insert("1.0", hotkeys_info)
        ...
        # Кнопка закрытия
        close_btn = ttk.Button(hotkeys_window, text="Закрыть", command=hotkeys_window.destroy)
